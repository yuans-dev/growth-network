...
const role = (process.argv[4] || "advisor").toLowerCase();

if (!email) {
  console.error(
    "Usage: node scripts/create-account.mjs <email> [password] [role]",
  );
  process.exit(1);
}

if (!["advisor", "admin", "staff"].includes(role)) {
  console.error("Role must be one of: advisor, admin, staff");
  process.exit(1);
}
...
async function main() {
  ...
  console.log(
    `Created/updated user: ${email} -> role: ${role} (id: ${user.id})`,
  );
}
